...
- (void)customAudioConfig {
    ...
    NSURL *url = [path URLByAppendingPathComponent:@"audioRecord.aac"];
...
void bufferReady(void *inUserData,AudioQueueRef inAQ,
    ...
    KJAACDecoder* player = (__bridge KJAACDecoder *)inUserData;
    if (!player) {
        NSLog(@"player nil");
        return;
    }
...
- (bool)fillBuffer:(AudioQueueBufferRef)buffer {
    ...
    NSAssert(status == noErr, ([NSString stringWithFormat:@"error status %d", (int)status]) );
    if (packets > 0) {
...
- (void)stop {
    AudioQueueStop(audioQueue, NO);// Stop playing or recording audio.
}
