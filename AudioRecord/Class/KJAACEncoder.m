...
- (void) setupEncoderFromSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    outAudioStreamBasicDescription.mSampleRate = 8000.0f;//inAudioStreamBasicDescription.mSampleRate; // 音频流，在正常播放情况下的帧率。如果是压缩的格式，这个属性表示解压缩后的帧率。帧率不能为0。
...
{
    KJAACEncoder *encoder = (__bridge KJAACEncoder *)(inUserData);
...
- (void) encodeSampleBuffer:(CMSampleBufferRef)sampleBuffer completionBlock:(void (^)(NSData * encodedData, NSError* error))completionBlock {
    ...
    dispatch_async(_encoderQueue, ^{
        ...
        NSLog(@"PCM Buffer Size: %zu", self->_pcmBufferSize);
